...
const SignPage = () => {
 ...
 const showpassword = () => {
  ...
  const handlephone = (e) => {
    ...
    if(Phone.length>9 || Phone.length<9){setphoneerror("* Number must be 10 character")}
    if(Phone.length==10){setphoneerror("")}
     if(isNaN(Phone)){
  ...
  const handlesigin = (e) => {
    if (username.length === 0) { setusererror("* Username is required to sign up") }
    if (password.length === 0) { setpasswerror("* Password is required to sign up") }
    if (email.length === 0) { setemailerror("* Email is required to sign up") }
    if (Phone.length === 0) { setphoneerror("* Phone No is required to sign up") }
    if (username.length > 7 && password.length > 7 && email.length > 8 && Phone.length ===10 && !isNaN(Phone)) {
      ...
      setTimeout(() => {
        ...
        toast.success('Siging successfully!', {
          icon: '✅',
          style: {
            fontSize: '18px',
            borderRadius: '10px',
    ...
    <>
      ...
      <div className="container box w-150 good mx-auto bg-blue-900/20 mt-8 rounded-4xl ">
        ...
        <div className="logo">
          ...
          />
          <div className='error text-red-500 w-80  font-medium ml-22 mb-1'>{usererror}</div>
         ...
         <div className="relative"> <input
          ...
          />
           <img className='view invert absolute top-2 left-[85%]' width={32} src={ispassword? './view.png':'./hide.png'} alt="" onClick={()=>showpassword()}/>
           </div>
