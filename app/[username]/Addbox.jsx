                    ...
                    <div className='h1 animate-fade-in-left h-[80%] w-[92%] bg-slate-900/40 custom-scroll m-auto overflow-auto rounded-4xl'>
                        ...
                        {entries.map(entry => (
                            <div key={entry.id} className='w animate-fade-in-down flex justify-evenly mt-3 mx-3  py-2 rounded-3xl text-slate-100/90'>
                                <div className='w1 text w-[12.5%] flex flex-col items-center'>
                                    <span className=' text w-auto'>{entry.date}</span>
                                    <span className='text w-auto'>{entry.time}</span>
                                </div>
                              
                                    <div className='text w-[92%] margin overflow-x-auto'>{entry.description}</div>
                                    <div className='text1 amount flex items-center mx-10  text-red-500 font-semibold text-2xl'>₹{entry.amount}</div>
                            
                                <div className='editdelete flex gap-10 w-[12.5%] ml-2 '>
                                    <button onClick={() => handleedit(entry)}><img  src="./edit.gif" className='ed  w-10' alt="" /></button>
                                    <button onClick={() => handledelete(entry._id)}><img  src="./delete.gif" className='ed w-10' alt="" /></button>
                                </div>
                                <div></div>
                            </div>
                        ))};
